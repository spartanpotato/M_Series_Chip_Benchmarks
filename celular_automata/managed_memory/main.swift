...
func main() {
    ...
    // Prepare input and output buffers with storageModeManaged
    ...
    let gridSizeBuffer = device.makeBuffer(bytes: &gridSize, length: MemoryLayout<vector_uint2>.stride, options: .storageModeManaged)

    // Stencil radius buffer
    var stencilRadiusValue = stencilRadius
    let stencilRadiusBuffer = device.makeBuffer(bytes: &stencilRadiusValue, length: MemoryLayout<Int>.stride, options: .storageModeManaged)

    // Starts measuring energy
    if checkInstantEnergy == 1 {
        let ret = startInstantEnergyMeasurement(N: n, storageType: 1)
        if ret == 0 {
    ...
    if checkEnergyOverTime == 1 {
        let ret = startEnergyOverTimeMeasurement(N: n, storageType: 1)
        if ret == 0 {
            print ("Energy measurement started successfully.")
            usleep(1000000);
        } else {
    ...
    writeTimesToCSV(N: n, radius: stencilRadius, elapsedTime: elapsed_, accessPerMs: accessPerMs, storageType: 1)
}
